...
- whisper.cpp: official Windows builds are `whisper-bin-x64.zip` on the GitHub release (v1.9.3 and v1.9.4 had no Windows assets; v1.9.2 does). whisper-cli needs the Visual C++ runtime (MSVCP140, VCRUNTIME140(_1), VCOMP140), bundled from the `msvc-runtime` wheel; `build.py` fails if any bundled binary imports a DLL that is neither bundled nor part of Windows. whisper-cli exits 0 even when it cannot read the audio: check that the output files exist. Models come from Hugging Face `ggerganov/whisper.cpp`, pinned to a commit (the LFS oid is the SHA-256).
- Tesseract for Windows: UB Mannheim publishes only an NSIS installer; the build unpacks it with 7-Zip (never runs it) and keeps the 26 DLLs tesseract.exe imports. OCRmyPDF is not needed: pages are rendered with PDFium and Tesseract's `textonly_pdf=1` layer is laid over the untouched page.
- PDFium (pypdfium2) is not thread-safe: always use `_pdf.pdfium_document()` (global lock).
- pikepdf `Page.images` is deprecated: use `get_images()`. `Page.flatten_rotation()` makes overlays on rotated pages simple.
